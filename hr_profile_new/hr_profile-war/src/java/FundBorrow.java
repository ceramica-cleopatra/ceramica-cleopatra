...
public class FundBorrow implements Cloneable {
            ...
            if (jedis.hgetAll("msgs:" + hrEmpInfo.getEmpNo()) != null && !jedis.hgetAll("msgs:" + hrEmpInfo.getEmpNo()).isEmpty()) {
                ...
                for (String hrProfileMsgAsString : hrProfileMsgMap.values()) {
                    ...
                    try {
                        ...
                        if (hrProfileMsg.getEntityName().equals("HrBorrowFundRequestGuarantee1") || hrProfileMsg.getEntityName().equals("HrBorrowFundRequestGuarantee2") || hrProfileMsg.getEntityName().equals("HrBorrowFundRequestDeptMng") || hrProfileMsg.getEntityName().equals("HrBorrowFundRequestResponsible")) {
    ...
    public void saveFundBorrow(ActionEvent ae) {
        ...
        try {
            ...
            hrEmpManagersMap = jedis.hgetAll("managers:" + hrEmpInfo.getEmpNo());
        } finally {
            if (jedis != null) {
                jedis.close();
            }
        ...
        for (String hrEmpManagersAsString : hrEmpManagersMap.values()) {
            if (hrBorrowFundRequest.getDeptMng().getEmpNo() == Long.parseLong(hrEmpManagersAsString)) {
                cnt++;
                break;
            }
        }
    ...
    public void cancelMessage(long empNo, int type) {
        ...
        try {
            ...
            if (jedis.hgetAll("alerts:" + empNo) != null && !jedis.hgetAll("alerts:" + empNo).isEmpty()
                    && !jedis.hgetAll("alerts:" + hrBorrowFundRequest.getDeptMng().getEmpNo()).isEmpty()) {
                ObjectMapper objectMapper = new ObjectMapper();
    ...
    public void sendMessages(Long reciever, int type) {
        ...
        try {
            ...
            try {
                ...
                if (type == 1 || type == 2) {
                    try {
                        jedis.hset("alerts:" + reciever, hrProfileMsg.getEntityName() + "" + hrProfileMsg.getMsgType() + "" + hrProfileMsg.getEmpNo() + "" + hrProfileMsg.getMsgId(), new ObjectMapper().writeValueAsString(hrProfileMsg));
