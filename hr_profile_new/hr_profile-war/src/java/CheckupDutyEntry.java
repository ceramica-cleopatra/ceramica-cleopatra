...
public class CheckupDutyEntry {
    ...
    public void save(ActionEvent ae) {
        ...
        Long chk = sessionBean.chkCheckupDuty2(hrEmpInfo.getEmpNo(), hrCheckupDutyHd.getTrnsDate(), brnId);

        if (chk == 1) {
            fc.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Œÿ√", " „  Ã«Ê“ «·Õœ «·√ﬁ’Ï «·„”„ÊÕ »Â ·≈œŒ«·  ﬁ«—Ì— „—Ê—"));
            return;
        }

        if (chk == 2) {
            fc.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Œÿ√", "«·„œ… «·“„‰Ì… „‰ √Œ— “Ì«—… ·Â–« «·„Êﬁ⁄ ·„   Ã«Ê“ «·› —… «·„œ… «·„ÿ·Ê»… »Ì‰ “Ì«— Ì‰ ·‰›” «·„Êﬁ⁄"));
            return;
        }
    ...
    public String sendToApprove() {
        ...
        try {
            ...
            for (Long l : mng) {
                ...
                if (CashHandler.getAlerts().containsKey(l)) {
                    CashHandler.getAlerts().get(l).add(hrProfileMsg);
                } else {
                    List<HrProfileMsg> hrProfileMsgs = new ArrayList<HrProfileMsg>();
                    hrProfileMsgs.add(hrProfileMsg);
                    CashHandler.getAlerts().put(l, hrProfileMsgs);
                }
