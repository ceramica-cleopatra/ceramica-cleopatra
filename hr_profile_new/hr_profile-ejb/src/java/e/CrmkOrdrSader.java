...
@NamedQueries({
    @NamedQuery(name = "CrmkOrdrSader.findNotApproved", query = "SELECT c FROM CrmkOrdrSader c where c.empApprove='Y' and (c.managerApprove is null or c.managerApprove='N') and c.brnId.id in (select l.crmkId from HrLocation l where l.id=:loc_id) and (c.canceled is null or c.canceled='N')"),
