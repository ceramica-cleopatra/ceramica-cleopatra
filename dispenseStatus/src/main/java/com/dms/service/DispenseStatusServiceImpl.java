...
public class DispenseStatusServiceImpl implements DispenseStatusService {
	...
	public TrnsOrdrDTO getOrdrDispenseStatus(Long trnsOrdrId) {
		...
		ordrMap.forEach((k, v) -> {
				...
				ordrDTO.setOrdrType(v.get(0).getCrmkSehy()!=null? (v.get(0).getCrmkSehy().equals("C") ? "سيراميك"
						: (v.get(0).getCrmkSehy().equals("D") ? "ديكور" : "صحى")) : null);
				ordrDTO.setPrdId(v.get(0).getPrdId());
				ordrDTO.setClntName(v.get(0).getClntName());
				ordrDTO.setTrnsDate(v.get(0).getTrnsDate()!=null ? simpleDateFormat.format(v.get(0).getTrnsDate()) : null);
				ordrDTO.setItemList(new ArrayList<>());
				if(ordrDTO.getDispenseStatus().equals("NotDispatched") || ordrDTO.getDispenseStatus().equals("PartiallyDispatched")){
					v.forEach(item -> {
