...
class CrmkSearchResultTable extends StatefulWidget {
  ...
  String govId;
  @override
  _CrmkSearchResultState createState() => _CrmkSearchResultState();
}

class _CrmkSearchResultState extends State<CrmkSearchResultTable> {
  _CrmkSearchResultState();
  ...
  void initState() {
    ...
    EasyLoading.init();
    val = _fillCrmkItemDetails(widget.typeId, widget.sizeId, widget.factoryNo,
        widget.dekalaId, widget.frz, widget.govId);
  }
  ...
  setSelectedPListRadio(Object val) {
    setState(() {
      ...
      crmkPlistNo = (val as CrmkPlist).no.toString();
    });
  }

  Future<void> setPriceForCurrentPlist(String priceListId) async {
    Price priceModel = await PriceService.getCrmkPrice(
        widget.typeId,
        widget.sizeId,
        widget.dekalaId,
        widget.factoryNo,
  ...
  setSelectedDListRadio(Object val) {
    setState(() {
      ...
      crmkDlistNo = (val as CrmkDlist).no.toString();
    });
  }
  ...
  Future<void> getPlList() async {
    pList = await CrmkService.getCrmkPlist();
    for (CrmkPlist list in pList) {
      if (list.id.toString() == pListId) {
        setState(() {
          selectedPList = list;
          crmkPlistNo = list.no.toString();
        });
      }
    }
  }
  ...
  Future<void> getDlList() async {
    dList = await CrmkService.getCrmkDlist();
    for (CrmkDlist list in dList) {
      if (list.id.toString() == dListId) {
        setState(() {
          selectedDList = list;
          crmkDlistNo = list.no.toString();
        });
      }
    }
  }

  Future<int> _fillCrmkItemDetails(String typeId, String sizeId,
      String factoryNo, String dekalaId, String frz, String govId) async {
    columns = [];
    rows = [];
    EasyLoading.show(status: 'جارى البحث...');
    isLoadingFlag = true;
    List<CrmkGovern> governList = await CrmkService.getCrmkDetails(
        typeId, sizeId, dekalaId, factoryNo, frz, govId);
    isLoadingFlag = false;
    EasyLoading.dismiss();
